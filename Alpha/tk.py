...
def InsertMainFrame():
    ...
    btnRep = Button(frame, text='Составить Отчет', width=18, height=4, font=('Times New Roman', '18'), bg='#e0905e', command=lambda:[Clear(frame), InsertReportFrame(), db.StartReport()])
...
def InsertIngrFrame():
    ...
    btn_Done = Button(frame, text='Добавить', width=12, height=2, font=('Times New Roman', '14'), bg='#e0905e', command=lambda:[db.AddIngredient(ent_Ingr.get(), ent_Price.get()),Refresh()])
    btn_Done.grid(row=3, column=1)

def InsertDishFrame():
    print(db.StartDishPars())

    def Refresh():
        entID.delete(0, END)
        entDish1.delete(0, END)
        entDish2.delete(0, END)
    ...
    entDish1 = Entry(frame, width=18, font=('Times New Roman', '14'))

    lblDish.grid(row=2, column=1, padx=200)
    entDish1.grid(row=3, column=1)

    lblDish = Label(frame, text='Кол-во для Яслей:', width=14, height=2, font=('Times New Roman', '14'), bg='#8fb5f2')
    entDish2 = Entry(frame, width=18, font=('Times New Roman', '14'))

    lblDish.grid(row=4, column=1, padx=10)
    entDish2.grid(row=5, column=1)

    btn_Done = Button(frame, text='Добавить\nингредиент\nв блюдо', width=12, height=3, font=('Times New Roman', '14'), bg='#e0905e', command=lambda:[db.AddIngrToDish(entID.get(),entDish1.get(), entDish2.get()), Refresh()])
    btn_Done.grid(row=6, column=1, pady=20)

    btn_Done = Button(frame, text='Добавить\nблюдо', width=12, height=3, font=('Times New Roman', '14'), bg='#e0905e', command=lambda:[db.EndDishPars(entDish.get()), Clear(frame), InsertDishFrame()])
...
def InsertReportFrame():
    ...
    def Refresh():
        entDish1.delete(0, END)
        entDish2.delete(0, END)
        entDish3.delete(0, END)
        entDish4.delete(0, END)
        entDish5.delete(0, END)
        entDish6.delete(0, END)
    ...
    entDish1 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish1.grid(row=2, column=1)
    entDish2 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish2.grid(row=2, column=2)
    entDish3 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish3.grid(row=2, column=3)
    entDish4 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish4.grid(row=3, column=1)
    entDish5 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish5.grid(row=3, column=2)
    entDish6 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish6.grid(row=3, column=3)
    
    btn_Done = Button(frame, text='Добавить\nблюдо на\nзавтрак', width=15, height=3, font=('Times New Roman', '13'), bg='#e0905e', command=lambda:[db.ReportDishesFill(isGarten,0,entDish1.get(),entDish4.get()), Refresh()])
    btn_Done.grid(row=4, column=1)
    btn_Done = Button(frame, text='Добавить\nблюдо на\nобед', width=15, height=3, font=('Times New Roman', '13'), bg='#e0905e', command=lambda:[db.ReportDishesFill(isGarten,1,entDish2.get(),entDish5.get()), Refresh()])
    btn_Done.grid(row=4, column=2)
    btn_Done = Button(frame, text='Добавить\nблюдо на\nужин', width=15, height=3, font=('Times New Roman', '13'), bg='#e0905e', command=lambda:[db.ReportDishesFill(isGarten,2,entDish3.get(),entDish6.get()), Refresh()])
    ...
    entDish7 = Entry(frame, width=17, font=('Times New Roman', '14'))
    entDish7.grid(row=5, column=3)

    btn_Done = Button(frame, text='Создать отчет', width=16, height=2, font=('Times New Roman', '14'), bg='#e0905e', command=lambda:[db.FillReport(entDish7.get()), Clear(frame), InsertMainFrame()])
